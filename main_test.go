...
func TestFormat(t *testing.T) {
	for _, tc := range formatTests {
		t.Run(tc.name, func(t *testing.T) {
			...
			input := fileContent(a, "input.lean")
			expected := fileContent(a, "output.lean")

			// txtar adds trailing newlines to files. We strip them from both
			// input and expected so the test reflects pure content comparison.
